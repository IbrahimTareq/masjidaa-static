...
export default function AnnouncementsClient({
  ...
  const getStatusBadge = (announcement: Tables<"announcements">) => {
    ...
    if (announcement.pinned) {
      badges.push(
        <span key="pinned" className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-theme-accent text-theme-gradient">
          <Pin className="w-3 h-3 mr-1" fill="currentColor" stroke="currentColor" />
          Pinned
        </span>
      );
    }
    ...
    if (createdDate > weekAgo) {
      badges.push(
        <span key="added" className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-green-100 text-green-800">
    ...
    if (announcement.updated_at && 
      ...
      if (updatedDate > weekAgo) {
        badges.push(
          <span key="updated" className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
  ...
  return (
    <div className="bg-white text-black min-h-screen">
      ...
      <section className="relative py-20 lg:py-20 overflow-hidden">
        <div className="absolute inset-0 bg-[url('/pattern8.jpg')] bg-repeat opacity-10" style={{ backgroundSize: "400px" }} />
        <div className="relative max-w-7xl mx-auto px-4 lg:px-0">
          <div className="max-w-3xl">
            ...
            <p className="text-xl text-gray-600">
              Stay updated with the latest news, announcements, and important information from {masjid.name}.
            </p>
          </div>
        </div>
      </section>
            ...
            <div className="space-y-6">
              {[...announcements]
                ...
                return (
                  <Link
                    key={announcement.id}
                    href={`/${masjid.slug}/announcement/${announcement.id}`}
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            {announcement.description && (
                              <p className="text-gray-600 leading-relaxed mb-4 overflow-hidden text-ellipsis" style={{
                                display: '-webkit-box',
                                WebkitLineClamp: 3,
