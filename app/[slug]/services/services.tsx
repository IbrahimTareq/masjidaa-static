          ...
          {services && services.length > 0 ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
              {services.map((service, index) => (
                <div
                  key={index}
                  className="bg-white rounded-lg shadow-lg p-6 md:p-8 hover:shadow-xl transition-shadow duration-300 flex flex-col"
                >
                  <div className="text-theme-gradient mb-4">
                    <Info className="w-10 h-10" />
                  </div>
                  <h3 className="text-xl md:text-2xl font-bold mb-4">{service.title}</h3>
                  <p className="text-sm md:text-base text-gray-600 mb-6">{service.description}</p>
                  {service.cta && (
                    <a
                      href={service.cta.link}
                      className="mt-auto inline-flex items-center justify-center min-h-[44px] px-6 py-3 text-theme-gradient border border-theme-gradient/20 rounded-md font-medium hover:bg-theme-gradient hover:text-white transition-all duration-200 group"
                    >
                      <span className="truncate">{service.cta.text}</span>
                      <svg
